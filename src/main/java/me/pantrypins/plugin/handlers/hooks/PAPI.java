...
import static me.pantrypins.plugin.utils.Utils.formatNumber;

public class PAPI extends PlaceholderExpansion {

    private final Map<String, Tag> tags;

    public PAPI(PantryPins plugin) {
    ...
    public @NotNull String getVersion() {
        return PantryPins.getInstance().getDescription().getVersion();
    }
    ...
    public String onRequest(OfflinePlayer player, @NotNull String params) {
        ...
        switch (params.toLowerCase()) {
            ...
            case "player_track_unlocked": {
                ...
                if (!player.isOp()) {
                    ...
                    count = PantryPins.getInstance().getTagManager().getTags().size();
                }
            ...
            case "tags_amount":
            case "tags_total":
                return String.valueOf(PantryPins.getInstance().getTagManager().getTags().size());
        }

        // Dynamic placeholders
        if (params.startsWith("has_access_")) {
            String identifier = params.substring("has_access_".length());
            Tag tag = PantryPins.getInstance().getTagManager().getTag(identifier);
            if (tag != null) {
        ...
        if (params.startsWith("tag_custom-placeholder_")) {
            ...
            Tag tag = PantryPins.getInstance().getTagManager().getTag(activeTagId);

            if (tag == null && PantryPins.getInstance().getTagManager().isVariant(activeTagId)) {
                tag = PantryPins.getInstance().getTagManager().getVariant(activeTagId).getSisterTag();
            }
        ...
        if (tag == null) {
            tag = PantryPins.getInstance().getPlayerManager().loadAllPlayerTags(player.getUniqueId()).get(activeTagId);
        }

        if (tag != null) {
            text = getTagInfo(tag, params, player, activeTagId);
        } else {
            Variant variant = PantryPins.getInstance().getTagManager().getVariantTag(player);
            if (variant != null) {
                text = getVariantInfo(variant, params, player, activeTagId);
            } else {
                if (params.equalsIgnoreCase("tag")) {
                    if (activeTagId.equalsIgnoreCase("None")) {
                        return PantryPins.getInstance().getConfig().getString("placeholders.tag.none-output", "");
                    }
                }

                if (params.equalsIgnoreCase("chattag")) {
                    if (activeTagId.equalsIgnoreCase("None")) {
                        return PantryPins.getInstance().getConfig().getString("placeholders.chat.none-output", "");
                    }
                }

                if (params.equalsIgnoreCase("tabtag")) {
                    if (activeTagId.equalsIgnoreCase("None")) {
                        return PantryPins.getInstance().getConfig().getString("placeholders.tab.none-output", "");
                    }
                }

                if (params.equalsIgnoreCase("scoreboardtag")) {
                    if (activeTagId.equalsIgnoreCase("None")) {
                        return PantryPins.getInstance().getConfig().getString("placeholders.scoreboard.none-output", "");
                    }
                }
            }
        }
    ...
    private String getTagInfo(Tag t, String params, OfflinePlayer player, String activeTagId) {
        switch (params.toLowerCase()) {
            case "tag":
                ...
                String formatted = PantryPins.getInstance().getConfig().getString("placeholders.tag.format");
            ...
            case "chattag":
                ...
                String formatted2 = PantryPins.getInstance().getConfig().getString("placeholders.chat.format");
            ...
            case "scoreboardtag":
                ...
                String sbformatted = PantryPins.getInstance().getConfig().getString("placeholders.scoreboard.format");
            ...
            case "tabtag":
                ...
                String tabformatted = PantryPins.getInstance().getConfig().getString("placeholders.tab.format");
            ...
            case "rarity":
                return PantryPins.getInstance().getRarityManager().getRarity(t.getRarity()).getDisplayname();
            case "rarity_raw":
                return t.getRarity();
            case "category":
    ...
    private String getVariantInfo(Variant v, String params, OfflinePlayer player, String activeTagId) {
        ...
        if (params.equalsIgnoreCase("tag")) {
            if (activeTagId.equalsIgnoreCase("None")) {
                text = PantryPins.getInstance().getConfig().getString("placeholders.tag.none-output", "");
                return text;
            }
        ...
        if (params.equalsIgnoreCase("chattag")) {
            if (activeTagId.equalsIgnoreCase("None")) {
                text = PantryPins.getInstance().getConfig().getString("placeholders.chat.none-output", "");
                return text;
            }

            String tag = v.getTag().get(0);
            String formatted = PantryPins.getInstance().getConfig().getString("placeholders.chat.format");
            formatted = formatted.replace("%tag%", tag);
        ...
        if (params.equalsIgnoreCase("tabtag")) {
            if (activeTagId.equalsIgnoreCase("None")) {
                text = PantryPins.getInstance().getConfig().getString("placeholders.tab.none-output", "");
                return text;
            }

            String tag = v.getTag().get(0);
            String formatted = PantryPins.getInstance().getConfig().getString("placeholders.tab.format");
            formatted = formatted.replace("%tag%", tag);
        ...
        if (params.equalsIgnoreCase("scoreboardtag")) {
            if (activeTagId.equalsIgnoreCase("None")) {
                text = PantryPins.getInstance().getConfig().getString("placeholders.scoreboard.none-output", "");
                return text;
            }

            String tag = v.getTag().get(0);
            String formatted = PantryPins.getInstance().getConfig().getString("placeholders.scoreboard.format");
            formatted = formatted.replace("%tag%", tag);
        ...
        if (params.equalsIgnoreCase("rarity")) {
            return PantryPins.getInstance().getRarityManager().getRarity(v.getRarity()).getDisplayname();
        }
