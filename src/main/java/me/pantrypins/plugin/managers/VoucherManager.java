...
public class VoucherManager {
    ...
    private FileConfiguration tags = PantryPins.getInstance().getConfigManager().getConfig("tags.yml").get();
    ...
    public VoucherManager() {
        if (!PantryPins.getInstance().isFoliaFound()) {
            ...
            }.runTaskTimerAsynchronously(PantryPins.getInstance(), 0, 20 * 3); // Run every 3 seconds
        } else {
            ...
            try {
                ...
                runAtFixedRateMethod.invoke(scheduler, PantryPins.getInstance(), task, 0L, 60L); // Runs every 60 ticks (3 seconds)
            } catch (Exception e) {
                PantryPins.getInstance().getLogger().warning("Folia scheduler not found: " + e.getMessage());
            }
        }
    ...
    public void withdrawTag(Player player, String tag_name) {
        ...
        Tag t = PantryPins.getInstance().getTagManager().getTag(tag_name);
    ...
    public void giveVoucher(Player player, String name) {
        Tag t = PantryPins.getInstance().getTagManager().getTag(name);
