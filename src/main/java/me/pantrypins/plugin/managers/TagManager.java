...
import me.pantrypins.plugin.storage.TagData;
...
public class TagManager {
    ...
    private void createTagInternal(String identifier, String material, String tagText, List<String> description, String permission, double cost, int modelData, CommandSender sender) {
        ...
        String defaultCategory = PantryPins.getInstance().getConfig().getString("settings.default-category");
    ...
    public void validateTags(boolean from_tags_list) {
        if (from_tags_list) {
            for (Tag tag : tags.values()) {
                ...
                // Category check
                String category = tag.getCategory() != null ? tag.getCategory() : PantryPins.getInstance().getConfig().getString("settings.default-category");
            ...
            for (String identifier : getTagConfig().getConfigurationSection("tags").getKeys(false)) {
                ...
                // Category check
                if (!getTagConfig().isSet(basePath + ".category")) {
                    getTagConfig().set(basePath + ".category", PantryPins.getInstance().getConfig().getString("settings.default-category"));
                }
    ...
    public void setCategory(CommandSender sender, String identifier, String category) {
        ...
        if (!PantryPins.getInstance().getCategoryManager().getCatorgies().contains(category)) {
        ...
        try {
            PantryPins.getInstance().getTagManager().getTagConfig().set("tags." + identifier + ".category", t.getCategory());
            PantryPins.getInstance().getTagManager().saveTagConfig();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void saveTagConfig() {
        PantryPins.getInstance().getConfigManager().saveConfig("tags.yml");
    }
    ...
    public void reloadTagConfig() {
        PantryPins.getInstance().getConfigManager().reloadConfig("tags.yml");
    }
    ...
    public boolean isDBTags() {
        return PantryPins.getInstance().isDBTags();
    }
