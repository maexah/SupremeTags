...
import me.pantrypins.plugin.api.PantryPinsAPI;
import me.pantrypins.plugin.checkers.Metrics;
import me.pantrypins.plugin.commands.MyTags;
import me.pantrypins.plugin.commands.tags.TagsCommand;
import me.pantrypins.plugin.handlers.Editor;
import me.pantrypins.plugin.handlers.SetupTag;
import me.pantrypins.plugin.handlers.hooks.EssentialsChatListener;
import me.pantrypins.plugin.handlers.hooks.PAPI;
import me.pantrypins.plugin.handlers.menu.MenuUtil;
import me.pantrypins.plugin.managers.*;
import me.pantrypins.plugin.storage.*;
import me.pantrypins.plugin.storage.tags.MySQLTags;
import me.pantrypins.plugin.storage.tags.SQLiteTags;
...
import me.pantrypins.plugin.utils.ClassRegistrationUtils;
import me.pantrypins.plugin.utils.commands.CommandFramework;
...
public final class PantryPins extends AxPlugin {
    ...
    private static PantryPins instance;
    ...
    private static PantryPinsAPI api;
    ...
    public void onDisable() {
        ...
        if (isProtocolLib()) {
            try {
                Class<?> clazz = Class.forName("me.pantrypins.plugin.handlers.packets.ProtocolLibHandler");
        ...
        if (isPacketEvents()) {
            try {
                Class<?> clazz = Class.forName("me.pantrypins.plugin.handlers.packets.PacketEventsHandler");
                Object handler = clazz.getConstructor(Plugin.class).newInstance(this);
                clazz.getMethod("unRegister").invoke(handler);
            } catch (Exception e) {
    ...
    private void init() {
        ...
        ClassRegistrationUtils.loadListeners("me.pantrypins.plugin.listeners", this);
    ...
    public static PantryPins getInstance() { return instance; }
    ...
    private void sendConsoleLog() {
        ...
        logger.info("");
        logger.info("  ____             _              ____  _           ");
        logger.info(" |  _ \\ __ _ _ __ | |_ _ __ _   _|  _ \\(_)_ __  ___ ");
        logger.info(" | |_) / _` | '_ \\| __| '__| | | | |_) | | '_ \\/ __|");
        logger.info(" |  __/ (_| | | | | |_| |  | |_| |  __/| | | | \\__ \\");
        logger.info(" |_|   \\__,_|_| |_|\\__|_|   \\__, |_|   |_|_| |_|___/");
        logger.info("                            |___/                   ");
        logger.info(" PantryPins - Tags for your server.");
        logger.info("");
        if (dev_build) {
            logger.info("You're using a development build of PantryPins! build: #" + build);
        }
        logger.info("");
        ...
        logger.info("> Forked from SupremeTags by Scape.");
    ...
    public static PantryPinsAPI getTagAPI() {
        return api;
    }
    ...
    private void validateDefaultSounds() {
        ...
        PantryPins.getInstance().saveConfig();
    }
