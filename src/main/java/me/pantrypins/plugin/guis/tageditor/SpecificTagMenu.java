...
import me.pantrypins.plugin.guis.confirm.ConfirmationMenu;
...
public class SpecificTagMenu extends Menu {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (e.getSlot() == 13) {
            ...
            PantryPins.getInstance().getEditorList().put(player, editor);
            ...
            PantryPins.getInstance().getTagManager().saveTag(t);
            ...
            PantryPins.getInstance().getEditorList().put(player, editor);
            msgPlayer(player, rarity.replace("%current%", t.getRarity()), "&7Rarities: " + PantryPins.getInstance().getRarityManager().getRarityMap().keySet().toString().replace("[", "").replace("]", ""));
            player.closeInventory();
        } else if (e.getSlot() == 49) {
            String identifier = menuUtil.getIdentifier();
            player.closeInventory();
            new ConfirmationMenu(PantryPins.getMenuUtil(player), "delete-tag:" + identifier).open();
        } else {
            e.setCancelled(true);
        }
    }
    ...
    public void setMenuItems() {
        if (menuUtil.getIdentifier() != null) {
            if (PantryPins.getInstance().getTagManager().getTag(menuUtil.getIdentifier()) != null) {
                Tag t = PantryPins.getInstance().getTagManager().getTag(menuUtil.getIdentifier());
