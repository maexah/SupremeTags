package me.pantrypins.plugin.guis;
...
public class MainMenu extends Menu {
    ...
    public MainMenu(MenuUtil menuUtil) {
        super(menuUtil);
        this.catorgies = PantryPins.getInstance().getCategoryManager().getCatorgies();
        this.categoriesTags = PantryPins.getInstance().getCategoryManager().getCatorgiesTags();
    }
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (category != null) {
            int slot = PantryPins.getInstance().getCategoryManager().getCatConfig().getInt("categories." + category + ".slot");
            String permission = PantryPins.getInstance().getCategoryManager().getCatConfig().getString("categories." + category + ".permission");

            String prefix = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix");
            String no_access_category = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.no-access-category").replace("%prefix%", prefix);
            String no_tags_category = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.no-tags-category").replace("%prefix%", prefix);
            ...
            if (e.getSlot() == slot) {
                if (hasMinTags) {
                    if (permission != null && player.hasPermission(permission)) {
                        ...
                        new CategoryMenu(PantryPins.getMenuUtil(player)).open();
                    } else {
    ...
    public void setMenuItems() {
        for (String cats : getCatorgies()) {
            if (cats != null) {
                boolean canSee = PantryPins.getInstance().getCategoryManager().getCatConfig().getBoolean("categories." + cats + ".permission-see-category");
                boolean glow = PantryPins.getInstance().getCategoryManager().getCatConfig().getBoolean("categories." + cats + ".glow");
                String permission = PantryPins.getInstance().getCategoryManager().getCatConfig().getString("categories." + cats + ".permission");
                String material = PantryPins.getInstance().getCategoryManager().getCatConfig().getString("categories." + cats + ".material");
                int slot = PantryPins.getInstance().getCategoryManager().getCatConfig().getInt("categories." + cats + ".slot");
                int custom_model_data = PantryPins.getInstance().getCategoryManager().getCatConfig().getInt("categories." + cats + ".custom-model-data");
                String displayname = PantryPins.getInstance().getCategoryManager().getCatConfig().getString("categories." + cats + ".id_display");
                ...
                if (permission != null && menuUtil.getOwner().hasPermission(permission) && canSee) {
                    ...
                    if (material.contains("hdb-")) {
                        ...
                        cat_item = SkullUtil.getSkullByBase64EncodedTextureUrl(PantryPins.getInstance(), id);
                        cat_itemMeta = cat_item.getItemMeta();
                        ...
                        cat_item = PantryPins.getInstance().getItemWithNexo(id);
                        cat_itemMeta = cat_item.getItemMeta();
                    } else {
                    ...
                    // Set lore
                    ArrayList<String> lore = (ArrayList<String>) PantryPins.getInstance().getCategoryManager().getCatConfig().getStringList("categories." + cats + ".lore");
        ...
        for (String name : guis.getConfigurationSection("gui.main-menu.custom-items").getKeys(false)) {
            ...
            if (enable) {
                ...
                if (!identifier.equalsIgnoreCase("None")) {
                    ...
                    item_displayname = item_displayname.replace("%identifier%", Objects.requireNonNull(PantryPins.getInstance().getConfig().getString("placeholders.tag.none-output")));
                }
                ...
                if (PantryPins.getInstance().getTagManager().getTag(identifier) != null) {
                    if (PantryPins.getInstance().getTagManager().getTag(identifier).getCurrentTag() != null) {
                        tag = PantryPins.getInstance().getTagManager().getTag(identifier).getCurrentTag();
                    } else {
                        tag = PantryPins.getInstance().getTagManager().getTag(identifier).getTag().get(0);
                    }
                } else {
                    tag = "";
                }
        ...
        if (PantryPins.getInstance().getCategoryManager().getCatConfig().getBoolean("categories-menu-fill-empty")) {
            fillEmpty();
        }
    }
