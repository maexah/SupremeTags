...
import me.pantrypins.plugin.handlers.menu.*;
import me.pantrypins.plugin.managers.TagManager;
import me.pantrypins.plugin.storage.*;
...
public class CategoryMenu extends Paged {
    ...
    private FileConfiguration categories = PantryPins.getInstance().getConfigManager().getConfig("categories.yml").get();

    public CategoryMenu(MenuUtil menuUtil) {
        super(menuUtil);
        tags = PantryPins.getInstance().getTagManager().getTags();
        dataItem = PantryPins.getInstance().getTagManager().getDataItem();
    ...
    public String getMenuName() {
        String title = format(Objects.requireNonNull(PantryPins.getInstance().getCategoryManager().getCatConfig().getString("categories." + menuUtil.getCategory() + ".title")).replace("%page%", String.valueOf(this.getPage())));
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        String insufficient = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.insufficient-funds").replace("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
        String unlocked = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.tag-unlocked").replace("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));

        String no_tag_selected = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.no-tag-selected").replace("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));;
        ...
        if (nbt.hasTag("identifier")) {
            ...
            boolean canDeactivate = PantryPins.getInstance().isDeactivateClick();
        ...
        if (nbt.hasTag("name")) {
            ...
            if (name.equalsIgnoreCase("reset")) {
                ...
                if (!PantryPins.getInstance().getConfig().getBoolean("settings.forced-tag")) {
                    ...
                    if (PantryPins.getInstance().getConfig().getBoolean("settings.gui-messages")) {
                        msgPlayer(player, PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.reset-message").replace("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix"))));
                    }
    ...
    public void getTagItemsCategory() {
        ...
        if (!tag.isEmpty()) {
            ...
            tag.sort(getTagComparator(PantryPins.getInstance().getConfig().getBoolean("settings.prioritise-selected-tag"), menuUtil));
            ...
            for (int i = startIndex; i <= endIndex; i++) {
                ...
                if (PantryPins.getInstance().getConfig().getBoolean("settings.only-show-player-access-tags")) {
                    if (!menuUtil.getOwner().hasPermission(permission) &&
                            (!PantryPins.getInstance().getConfig().getBoolean("settings.locked-view") &&
                                    !PantryPins.getInstance().getConfig().getBoolean("settings.cost-system"))) {
                        continue;
                    }
                }
                ...
                if (!t.getEffects().isEmpty()) {
                    String formatEffectTemplate = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.effects-replace-style");
                    ...
                    joinedEffects = format(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.no-effects"));
                    effects_list = format(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.no-effects"));
                }
