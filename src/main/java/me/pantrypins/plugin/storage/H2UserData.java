...
public class H2UserData {
    ...
    public boolean exists(Player player) {
        try {
            PreparedStatement statement = PantryPins.getH2Database().getConnection().prepareStatement("SELECT * FROM `users` WHERE (UUID=?)");
    ...
    public void createPlayer(Player player) {
        ...
        try (PreparedStatement statement = PantryPins.getH2Database().getConnection().prepareStatement(
    ...
    public static void setActive(OfflinePlayer player, String identifier) {
        ...
        try (PreparedStatement statement = PantryPins.getH2Database().getConnection().prepareStatement(sql)) {
    ...
    public static String getActive(UUID uuid) {
        ...
        try (Connection connection = PantryPins.getH2Database().getConnection();
