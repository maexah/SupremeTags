...
public class PlayerConfig {
    ...
    public static boolean exists(OfflinePlayer player) {
        return new File(PantryPins.getInstance().getDataFolder(), "data/" + player.getUniqueId() + ".yml").exists();
    }

    public static FileConfiguration get(OfflinePlayer offlinePlayer) {
        File file = new File(PantryPins.getInstance().getDataFolder(), "data/" + offlinePlayer.getUniqueId() + ".yml");
        return YamlConfiguration.loadConfiguration(file);
    }

    public static void saveConfigOnly(Player player) {
        File file = new File(PantryPins.getInstance().getDataFolder(), "data/" + player.getUniqueId() + ".yml");
    ...
    public static void save(Player player) {
        ...
        List<Tag> tags = PantryPins.getInstance().getPlayerManager().getPlayerTags().get(player.getUniqueId());
    ...
    public void loadPlayer(OfflinePlayer player) {
        if (exists(player)) {
            ...
            if (config.getConfigurationSection("tags") != null) {
                ...
                PantryPins.getInstance().getPlayerManager().getPlayerTags().put(player.getUniqueId(), tags);
            }
        } else {
            createFile(player);
            loadPlayer(player);
    ...
    void createFile(OfflinePlayer player) {
        File folder = new File(PantryPins.getInstance().getDataFolder(), "data");
        File file = new File(PantryPins.getInstance().getDataFolder(), "data/" + player.getUniqueId() + ".yml");
