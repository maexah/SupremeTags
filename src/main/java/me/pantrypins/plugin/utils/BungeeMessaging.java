...
public class BungeeMessaging implements PluginMessageListener {
    ...
    private static final Logger LOGGER = Logger.getLogger("PantryPins/PluginMessenger");
    ...
    public static void sendReload() {
        ...
        try (
            ...
            if (PantryPins.getInstance().isBungeeCord()) {
                ...
                player.sendPluginMessage(PantryPins.getInstance(), BUNGEE_CHANNEL, stream.toByteArray());
            } else {
                player.sendPluginMessage(PantryPins.getInstance(), VELOCITY_CHANNEL, msgBytes.toByteArray());
            }
    ...
    public void onPluginMessageReceived(@NotNull String channel, @NotNull Player player, byte[] message) {
        if (PantryPins.getInstance().isBungeeCord() && !channel.equalsIgnoreCase(BUNGEE_CHANNEL)) return;
        if (!PantryPins.getInstance().isBungeeCord() && !channel.equalsIgnoreCase(VELOCITY_CHANNEL)) return;
