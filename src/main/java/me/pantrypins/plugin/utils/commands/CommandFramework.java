...
public class CommandFramework implements CommandExecutor {
    ...
    public CommandFramework(PantryPins plugin) {
        this.plugin = plugin;
    ...
    public boolean handleCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                FileConfiguration messages = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get();

                if (!command.permission().equals("") && (!sender.hasPermission(command.permission()))) {
                    String no_perm_command_format = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.no-permission").replaceAll("%prefix%", Objects.requireNonNull(messages.getString("messages.prefix")));
                    msgPlayer(sender, no_perm_command_format);
                    return true;
                }

                if (command.inGameOnly() && !(sender instanceof Player)) {
                    String invalid_console_access_format = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.no-console-access").replaceAll("%prefix%", Objects.requireNonNull(messages.getString("messages.prefix")));
    ...
    public void registerCommands(Object obj, List<String> aliases) {
        for (Method method : obj.getClass().getMethods()) {
            ...
            if (method.getAnnotation(Command.class) != null) {
                ...
                if (command.name().equalsIgnoreCase(PantryPins.getInstance().getConfig().getString("settings.commands.main-command"))) {
                    aliases = new ArrayList<>(PantryPins.getInstance().getConfig().getStringList("settings.commands.aliases"));
                }
    ...
    public void registerCommand(Command command, @NotNull  String label, @NotNull Method m, @NotNull Object obj) {
        if (label.equalsIgnoreCase("tags")) {
            label = PantryPins.getInstance().getConfig().getString("settings.commands.main-command");
        }
        ...
        // Register aliases
        for (String alias : PantryPins.getInstance().getConfig().getStringList("settings.commands.aliases")) {
