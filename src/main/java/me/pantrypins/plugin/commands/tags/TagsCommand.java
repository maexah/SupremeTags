package me.pantrypins.plugin.commands.tags;
...
import me.pantrypins.plugin.guis.configeditor.ConfigOneMenu;
import me.pantrypins.plugin.guis.search.SearchResultMenu;
import me.pantrypins.plugin.guis.tageditor.EditorSelectorMenu;
...
import me.pantrypins.plugin.utils.BungeeMessaging;
import org.bukkit.*;
...
public class TagsCommand implements CommandExecutor, TabCompleter {
    ...
    private String noperm = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.no-permission").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    private String notags = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.no-tags").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    private String tagedited = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.tag-edited").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    private String commanddisabled = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.tag-command-disabled").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    private String invalidtag = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.invalid-tag").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    private String validtag = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.valid-tag").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    private String player_no_tag = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.player-not-have-tag").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    private String player_not_online = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.player-not-online").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    private String given_voucher = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.given-voucher").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    private String received_voucher = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.received-voucher").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    private String reset = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.reset-command").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    private String tag_removed = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.tag-removed").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
    ...
    private void handleEdit(CommandSender sender, String[] args) {
        ...
        if (!PantryPins.getInstance().getTagManager().doesTagExist(tag)) {
            msgPlayer(sender, invalidtag);
            return;
        }

        Tag t = PantryPins.getInstance().getTagManager().getTag(tag);
    ...
    private void handleMainCommand(CommandSender sender, Player player) {
        ...
        if (!PantryPins.getInstance().isNoPermissionMenuAction()) {
        ...
        if (!PantryPins.getInstance().isDisabledWorldsTag()) {
            boolean lockedView = PantryPins.getInstance().getConfig().getBoolean("settings.locked-view");
            boolean costSystem = PantryPins.getInstance().getConfig().getBoolean("settings.cost-system");
            ...
            for (String world : PantryPins.getInstance().getConfig().getStringList("settings.disabled-worlds")) {
    ...
    private void handleReload(CommandSender sender) {
        ...
        if (PantryPins.getInstance().getConfig().getBoolean("settings.bungee-messaging")) {
            BungeeMessaging.sendReload();
        }

        PantryPins.getInstance().reload();

        PantryPins.getInstance().getConfigManager().reloadConfig("messages.yml");
        msgPlayer(sender, PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.reload").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix"))));
    }
    ...
    private void handleConfig(CommandSender sender, Player player) {
        ...
        if (player != null) {
            new ConfigOneMenu(PantryPins.getMenuUtil(player)).open();
    ...
    private void handleList(CommandSender sender) {
        ...
        msgPlayer(sender,
                "&e&lTags &8➜ &7There are &f" + PantryPins.getInstance().getTagManager().getTags().size() + " &7tags loaded!",
                "&e&lTags &8➜ &7There are &f" + PantryPins.getInstance().getCategoryManager().getCatorgies().size() + " &7categories loaded!",
    ...
    private void handleEditor(CommandSender sender, Player player) {
        ...
        if (player != null) {
            new EditorSelectorMenu(PantryPins.getMenuUtil(player)).open();
            playConfigSound(player, "open-menus");
        } else {
            sender.sendMessage("Only players can use this command");
        }
    ...
    private void handleMerge(CommandSender sender, boolean isFree) {
        ...
        PantryPins.getInstance().getMergeManager().merge(sender, !isFree);
    }
    ...
    private void handleDelete(CommandSender sender, Player player, String[] args) {
        ...
        String name = args[1];
        PantryPins.getInstance().getTagManager().deleteTag(sender, name);
    }
    ...
    private void handleWithdraw(CommandSender sender, Player player, String[] args) {
        ...
        if (!PantryPins.getInstance().getTagManager().tagExists(tagName)) {
            msgPlayer(sender, invalidtag);
            return;
        }

        PantryPins.getInstance().getVoucherManager().withdrawTag(player, tagName);
    }
    ...
    private void resetPlayerTag(CommandSender sender, OfflinePlayer target) {
        boolean forced = PantryPins.getInstance().getConfig().getBoolean("settings.forced-tag");
        String defaultTag = PantryPins.getInstance().getConfig().getString("settings.default-tag", "None");

        String no_tag_selected = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.no-tag-selected").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
        ...
        if (target.isOnline()) {
            Tag t = PantryPins.getInstance().getTagManager().getTag(UserData.getActive(target.getUniqueId()));
            if (t != null) {
    ...
    private void handleCreate(CommandSender sender, String[] args) {
        ...
        if (args.length < 3) {
            msgPlayer(sender, "&cUsage: /" + PantryPins.getInstance().getConfig().getString("settings.commands.main-command") + " create <name> <tag>");
            playConfigSound((Player) sender, "error-message");
            return;
        }
        ...
        if (PantryPins.getInstance().getTagManager().tagExists(name)) {
        ...
        PantryPins.getInstance().getTagManager().createTag(sender, name, tag, desc, "supremetags.tag." + name, 100);
    }
    ...
    private void handleRemoveTagP(CommandSender sender, Player player, String[] args) {
        ...
        if (!PantryPins.getInstance().getTagManager().tagExists(tag)) {
            msgPlayer(sender, invalidtag);
            playConfigSound((Player) sender, "error-message");
            return;
        }

        String permission = PantryPins.getInstance().getTagManager().getTag(tag).getPermission();

        for (World world : Bukkit.getWorlds()) {
            if (PantryPins.getPermissions().playerHas(world.getName(), Bukkit.getOfflinePlayer(playerName), permission)) {
                PantryPins.getPermissions().playerRemove(world.getName(), Bukkit.getOfflinePlayer(playerName), permission);
                hasTag = true;
            }
        }
    ...
    private void handleGiveVoucher(CommandSender sender, String[] args) {
        ...
        if (PantryPins.getInstance().getTagManager().getTag(name) != null) {
            PantryPins.getInstance().getVoucherManager().giveVoucher(target, name);
            msgPlayer(sender, given_voucher.replace("%target%", target.getName()).replace("%identifier%", name).replace("%tag%", PantryPins.getInstance().getTagManager().getTag(name).getTag().getFirst()));

            if (!received_voucher.isEmpty()) {
                msgPlayer(target, received_voucher.replace("%identifier%", name).replace("%tag%", PantryPins.getInstance().getTagManager().getTag(name).getTag().getFirst()));
            }
        } else {
            msgPlayer(sender, invalidtag);
            playConfigSound((Player) sender, "error-message");
    ...
    private void handleSetCategory(CommandSender sender, String[] args) {
        ...
        PantryPins.getInstance().getTagManager().setCategory(sender, name, category);
    }
    ...
    private void handleSet(CommandSender sender, String[] args) {
        ...
        boolean tagExists = PantryPins.getInstance().getTagManager().getTags().containsKey(identifier);
        ...
        if (args.length == 2) {
            ...
            if (!player.hasPermission(PantryPins.getInstance().getTagManager().getTag(identifier).getPermission())) {
                sendLockedMessage(player);
                return;
            }

            UserData.setActive(player, identifier);
            String select = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.tag-select-message");
            if (select == null) {
                select = "&aYou have selected the tag %tag%"; // fallback value
            }

            select = select.replace("%prefix%", PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix"));
            select = replacePlaceholders(player, select);
            msgPlayer(player, select
                    .replace("%identifier%", identifier)
                    .replace("%tag%", PantryPins.getInstance().getTagManager().getTag(identifier).getTag().getFirst()));
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        List<String> aliases = PantryPins.getInstance().getConfig().getStringList("settings.commands.aliases");
        String mainCommand = PantryPins.getInstance().getConfig().getString("settings.commands.main-command", "tags");
        ...
        if (allCommands.contains(label)) {
            ...
            // Second argument completion for specific subcommands
                ...
                switch (firstArg) {
                    ...
                    case "withdraw":
                    case "edit":
                        completions.addAll(PantryPins.getInstance().getTagManager().getTags().keySet());
                        break;
                    case "set":
                        PantryPins.getInstance().getTagManager().getTags().values().stream()
            ...
            // Fourth argument completion for the givevoucher command (optional flag -s)
                ...
                if (args[0].equalsIgnoreCase("givevoucher")) {
                    ...
                    String tagName = args[1];
                    if (PantryPins.getInstance().getTagManager().tagExists(tagName)) {
                        completions.addAll(PantryPins.getInstance().getTagManager().getTag(tagName).getTag());
                    }
                }
            }
        }
    ...
    public boolean hasTags(Player player) {
        for (Tag tag : PantryPins.getInstance().getTagManager().getTags().values()) {
    ...
    public void sendHelp(CommandSender sender) {
        if (sender == null) {
            for (String msg : PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getStringList("messages.help.admin")) {
                msgPlayer(Bukkit.getConsoleSender(), msg.replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix"))).replaceAll("%command%", PantryPins.getInstance().getConfig().getString("settings.commands.main-command")));
            }
            return;
        }
        ...
        if (sender instanceof Player player) {
            if (player.hasPermission("sc.admin")) {
                for (String msg : PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getStringList("messages.help.admin")) {
                    msgPlayer(player, msg.replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix"))).replaceAll("%command%", PantryPins.getInstance().getConfig().getString("settings.commands.main-command")));
                }
            } else {
                for (String msg : PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getStringList("messages.help.default")) {
                    msgPlayer(player, msg.replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix"))).replaceAll("%command%", PantryPins.getInstance().getConfig().getString("settings.commands.main-command")));
                }
            }
        } else {
            for (String msg : PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getStringList("messages.help.admin")) {
                msgPlayer(sender, msg.replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix"))).replaceAll("%command%", PantryPins.getInstance().getConfig().getString("settings.commands.main-command")));
            }
        }
    }
    ...
    public void sendDebug(CommandSender sender) {
        ...
        FileConfiguration msg = PantryPins.getInstance()
    ...
    private Map<String, String> collectDebugValues(CommandSender sender) {
        ...
        String version = PantryPins.getInstance().getDescription().getVersion();
        if (PantryPins.getInstance().dev_build) {
            version += "-DEV#" + PantryPins.getInstance().build;
        }

        map.put("%version%", version);
        map.put("%author%", "PantryPins (fork of SupremeTags by DevScape)");
        map.put("%discord%", "");

        map.put("%tags_loaded%", String.valueOf(PantryPins.getInstance().getTagManager().getTags().size()));
        map.put("%categories_loaded%", String.valueOf(PantryPins.getInstance().getCategoryManager().getCatorgies().size()));

        String dbType = PantryPins.getInstance()
        ...
        map.put("%hook_vault%", hookString(PantryPins.getInstance().isVaultAPI()));
        map.put("%hook_playerpoints%", hookString(isPlugin("PlayerPoints")));
        map.put("%hook_coinsengine%", hookString(PantryPins.getInstance().isCoinsEngine()));
        map.put("%hook_nbtapi%", hookString(isPlugin("NBTAPI")));
        map.put("%hook_papi%", hookString(PantryPins.getInstance().isPlaceholderAPI()));
    ...
    private List<String> debugTagErrorsCollect() {
        ...
        Collection<Tag> tags = PantryPins.getInstance().getTagManager().getTags().values();
    ...
    private List<String> validateMainConfigCollect() {
        ...
        FileConfiguration config = PantryPins.getInstance().getConfig();
        List<String> errors = new ArrayList<>();
    ...
    public void sendDebugOLD(CommandSender player) {
        msgPlayer(player, "");
        msgPlayer(player, "&fDebugging PantryPins &8➜");
        msgPlayer(player, "");
        msgPlayer(player, "&7Version: &f" + PantryPins.getInstance().getDescription().getVersion());
        msgPlayer(player, "&7Author: &fPantryPins (fork of SupremeTags by DevScape)");
        msgPlayer(player, "");
        msgPlayer(player, "&7Tags loaded: &f" + PantryPins.getInstance().getTagManager().getTags().size());
        msgPlayer(player, "&7Categories loaded: &f" + PantryPins.getInstance().getCategoryManager().getCatorgies().size());
        msgPlayer(player, "&7Database Assigned: &f" + PantryPins.getInstance().getConfigManager().getConfig("data.yml").get().getString("data.type"));
        ...
        if (PantryPins.getInstance().isVaultAPI()) {
        ...
        if (PantryPins.getInstance().isCoinsEngine()) {
            msgPlayer(player, " &8● &7CoinsEngine: &fFound.");
        } else {
            msgPlayer(player, " &8● &7CoinsEngine: &fNot found.");
        }

        if (PantryPins.getInstance().getServer().getPluginManager().getPlugin("NBTAPI") != null) {
    ...
    public void openSearchSign(Player player) {
        ...
        try {
            gui = SignGUI.builder()
                    .setLines(format(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.sign-line-top")), null, null)
                    ...
                    .setHandler((p, result) -> {
                        ...
                        if (!line1.isEmpty()) {
                            if (PantryPins.getInstance().getCategoryManager().isCategoryNearName(line1) || PantryPins.getInstance().getTagManager().tagExistsNearName(line1)) {
                                Bukkit.getScheduler().runTask(PantryPins.getInstance(), () -> new SearchResultMenu(PantryPins.getMenuUtil(player), line1).open());
                            } else {
                                String search_invalid = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.search-invalid-1").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
                                msgPlayer(player, search_invalid);
                            }
                        } else {
                            String search_invalid = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.search-invalid-2").replaceAll("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
                            msgPlayer(player, search_invalid);
                        }
    ...
    protected void sendLockedMessage(Player player) {
            String locked = PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.locked-tag")
                    .replace("%prefix%", Objects.requireNonNull(PantryPins.getInstance().getConfigManager().getConfig("messages.yml").get().getString("messages.prefix")));
